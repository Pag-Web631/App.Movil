...
const EnrollmentSchema = new mongoose.Schema({
  ...
  courseId: mongoose.Schema.Types.ObjectId,
  paymentMethod: { type: String, default: 'Sin método' },
  paymentStatus: { type: String, default: 'Pendiente' },
  approvalStatus: { type: String, default: 'Pendiente' }, // Pendiente / Aprobado / Rechazado
...
app.post('/api/enrollments', authenticateToken, async (req, res) => {
  try {
    ...
    const needsApproval = paymentMethod === 'Sin método';
    const enrollment = new Enrollment({
      studentId: req.user._id,
      courseId,
      paymentMethod: paymentMethod || 'Sin método',
      paymentStatus: needsApproval ? 'No Pagado' : 'Pagado',
      approvalStatus: needsApproval ? 'Pendiente' : 'Aprobado'
    });
...
app.patch('/api/enrollments/:id/approval', authenticateToken, async (req, res) => {
  try {
    ...
    if (action === 'approve') {
      ...
      enrollment.paymentStatus = enrollment.paymentMethod === 'Sin método' ? 'Aprobado sin pago' : 'Pagado';
