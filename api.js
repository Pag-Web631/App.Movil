...
class ApiClient {
  ...
  async enrollInCourse(courseId, paymentMethod = 'Sin método') {
    return await this.request('/enrollments', {
      method: 'POST',
  ...
  async updateEnrollmentApproval(enrollmentId, action, comment = '') {
    return await this.request(`/enrollments/${enrollmentId}/approval`, {
      method: 'PATCH',
